...
# ============================================================
# 1. Test MSE vs MC χ² Divergence - Value of Epsilon Fixed
# ============================================================

def plot_generalisation_gap_vs_chi_squared_mc_fixed_epsilon(
    df,
    plot_dir,
    epsilon,
    dimension=10,
    ...
    fig.suptitle(
        f"Generalisation Gap vs Monte-Carlo χ² Divergence  (d={dimension}, ε={epsilon}{target_str})",
        fontsize=12,
        y=1.02,
    )
    ...
    filename   = f"gen_gap_vs_mc_chi_sq_eps{eps_str}_d{dimension}{target_tag}.png"
 
    save_figure(filename, plot_dir)

# ==================================================================
# 2. Var(w(x)) vs MC χ² Divergence - Value of Epsilon Fixed
# ==================================================================

def plot_w_var_vs_chi_sq_mc_fixed_epsilon(
    df,
    plot_dir,
    epsilon,
    dimension=10,
...
def plot_ess_vs_chi_squared_mc_fixed_epsilon(
    df,
    plot_dir,
    epsilon,
    dimension=10,
    ...
    for ax, shift in zip(axes, SHIFT_TYPES):
        ...
        ax.xaxis.set_major_formatter(nice_x_formatter(subset["chi_sq_mid"]))
    ...
    target_str = f", target={target_mode}" if target_mode else ""
    fig.suptitle(
        f"Effective Sample Size vs Monte-Carlo χ² Divergence  (d={dimension}, ε={epsilon}{target_str})",
        fontsize=12,
        y=1.02,
    )
    ...
    eps_str    = str(epsilon).replace(".", "p")
    target_tag = f"_{target_mode}" if target_mode else ""
    filename   = f"ess_vs_mc_chi_sq_eps{eps_str}_d{dimension}{target_tag}.png"
 
    save_figure(filename, plot_dir)

# ==================================================================
# 4. MC χ² Divergence vs λ - Value of Epsilon Aggregated
# ==================================================================

def plot_chi_squared_mc_vs_lambda(
    df,
    plot_dir,
    target_mode=None,
    figsize=(16, 5),
    ...
    for d in DIMENSIONS:
        check_estimator_reliability(d, lambda_vals, SAMPLE_ESTIMATOR_EXPONENT_THRESHOLD)
    print()
    ...
    for ax, d in zip(axes, DIMENSIONS):
        ...
        if subset.empty:
            ...
            ax.set_ylabel("Monte-Carlo χ² Divergence", fontsize=10)
            continue
    ...
    fig.suptitle(
        f"Monte-Carlo χ² Divergence vs λ  (mean shift, aggregated over all ε{target_tag})",
        fontsize=12,
        y=1.02,
    )
 
    target_suffix = f"_{target_mode}" if target_mode else ""
    filename = f"mc_chi_squared_vs_lambda{target_suffix}.png"
 
    save_figure(filename, plot_dir)

# ==================================================================
# 5. MC χ² Divergence vs α - Value of Epsilon Aggregated
# ==================================================================

def plot_chi_squared_mc_vs_alpha(
    df,
    plot_dir,
    target_mode=None,
    alpha_max=None,
    ...
    target_tag = f", target={target_mode}" if target_mode else ""
    fig.suptitle(
        f"Monte-Carlo χ² Divergence vs α  (covariance shift, aggregated over all ε{alpha_str}{target_tag})",
        fontsize=12,
        y=1.02,
    )
    ...
    target_suffix = f"_{target_mode}" if target_mode else ""
    filename      = f"mc_chi_squared_vs_alpha{alpha_suffix}{target_suffix}.png"
